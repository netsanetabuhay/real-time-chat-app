      ...
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSubmit} action="" className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
            ...
            <img src={selectedImage ? URL.createObjectURL(selectedImage) : assets.avatar_icon} alt="" className={`w-12 h-12 ${selectedImage ? 'rounded-full' : ''}`} /> upload profile images
          </label>
          ...
          <textarea onChange={(e)=>setBio(e.target.value)} value={bio} required placeholder='write your bio from here' className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500 ' rows={4 }></textarea>
