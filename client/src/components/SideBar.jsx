...
const SideBar = () => {
  const {getUser, users, selectedUeser, setSelectedUserl unseenMessages, setUnseenMessages
  }   =  useContext(AuthContext);


  const {logout, onlineUsers} = useContext(AuthContext);

  cosnt [input, setInput] = useState(false);

    const navigate = useNavigate();

    const filterdUsers= input ? users.fileter((user)=>user.fullName.tolowerCase().includes(input.tolowerCase())):users;
